...
// ✅ Función fuera del POST — única definición
async function getAcceptanceToken() {
  try {
    console.log('Obteniendo acceptance token...');
    console.log('KEY primeros chars:', WOMPI_PUBLIC_KEY?.substring(0, 20));
    ...
    console.log('Status Wompi merchants:', response.status);
    const data = await response.json();
    console.log('Respuesta completa Wompi:', JSON.stringify(data));
